...
filtered_gitlogs <- my_gitlogs |>
  # set filter to two days after last relevant commit
  filter(!(repo == "rstudio-training" & time > ymd_h("2020-12-29 0"))) |>
  filter(!(repo == "software-resilience" & time > ymd_h("2021-02-23 0"))) |>
  filter(!(repo == "rtraining" & time > ymd_h("2021-10-09 0"))) |>
  filter(!(repo == "workshop7" & time > ymd_h("2021-12-08 0"))) |>
  filter(!(repo == "jbplot" & time > ymd_h("2022-02-08 0"))) |>
  # oldest first
...
```{r my_releases}
get_repo_releases <- function(repo, owner) {
  gh::gh("GET /repos/{owner}/{repo}/releases", owner = owner, repo = repo, .limit = Inf) |>
    purrr::map_df(~ list(repo = repo, name = .x$name, time = .x$published_at))
}

# cache results
if (!exists("my_releases")) {
  my_releases <- map_dfr(
    c("rtraining", "rdev", "workshop7", "jbplot", "siracon2022"),
    get_repo_releases, "jabenninghoff"
  ) |>
    arrange(time)
}
