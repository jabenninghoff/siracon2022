...
```{r monthly_commits_repo, fig.alt = "a multi-color line plot showing monthly commits by repository over time."}
filtered_gitlogs |>
...
```{r repo_timeline, fig.alt = "three parallel timelines showing when different repositories were actively developed."}
...
```{r key_events, fig.width = 10, fig.height = 18, fig.alt = "a timevis() timeline of key events from my SiRAcon 2022 presentation."}
...
```{r monthly_releases, fig.alt = "a line plot showing monthly GitHub releases over time, trending higher."}
...
```{r monthly_commits, fig.alt = "a line plot showing monthly git commits over time, trending higher."}
gitlogs |>
  filter(time < ymd_h("2022-05-01 0", tz = gitlogs_tz)) |>
  mutate(time = floor_date(time, unit = "month")) |>
  group_by(time) |>
...
```{r releases_per_commit, fig.alt = "a line plot showing monthly GitHub releases per commit over time, clearly trending higher."}
gitlogs |>
  filter(time < ymd_h("2022-05-01 0", tz = gitlogs_tz)) |>
  mutate(time = floor_date(time, unit = "month")) |>
  group_by(time) |>
...
```{r intro, fig.width = 8.6, fig.height = 4, fig.alt = "a timevis() timeline of key events from my SiRAcon 2022 presentation (Introduction)."}
key_events |>
  filter(group == 1) |>
...
```{r version_control, fig.width = 8.6, fig.height = 3, fig.alt = "a timevis() timeline of key events from my SiRAcon 2022 presentation (Version Control)."}
key_events |>
  filter(!intro) |>
...
```{r trunk_based, fig.width = 8.6, fig.height = 2.5, fig.alt = "a timevis() timeline of key events from my SiRAcon 2022 presentation (Trunk-based Development)."}
key_events |>
  filter(!intro) |>
...
```{r shift_left, fig.width = 8.6, fig.height = 2.5, fig.alt = "a timevis() timeline of key events from my SiRAcon 2022 presentation (Shift Left on Security)."}
key_events |>
  filter(!intro) |>
...
```{r ci, fig.width = 8.6, fig.height = 3.5, fig.alt = "a timevis() timeline of key events from my SiRAcon 2022 presentation (Continuous Integration)."}
key_events |>
  filter(!intro) |>
...
```{r deployments, fig.width = 8.6, fig.height = 4.5, fig.alt = "a timevis() timeline of key events from my SiRAcon 2022 presentation (Deployment Automation)."}
key_events |>
  filter(!intro) |>
...
```{r code_maint, fig.width = 8.6, fig.height = 5, fig.alt = "a timevis() timeline of key events from my SiRAcon 2022 presentation (Code Maintainability)."}
key_events |>
  filter(!intro) |>
...
```{r testing, fig.width = 8.6, fig.height = 3.5, fig.alt = "a timevis() timeline of key events from my SiRAcon 2022 presentation (Testing)."}
key_events |>
  filter(!intro) |>
