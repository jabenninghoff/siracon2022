...
key_events <- read_csv("data/key-events.csv", col_types = cols(
  ...
  mutate(id = row_number(), .before = "start")
...
filtered_gitlogs |>
  ...
  geom_vline(xintercept = ymd_h("2022-01-01 0", tz = Sys.timezone()), linetype = "dotted") +
  labs(title = "GitHub releases per commit") +
  labs(x = "", y = "") +
  theme_quo()
```
...
# TODO: move new_branch() to deployment automation, write_eval to code maintainability
# TODO: add events for each section in overall timeline
key_events |>
  filter(group == 1) |>
  timevis(groups = filter(dora_groups, id == 1))
```
