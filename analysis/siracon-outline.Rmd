...
ggsave("rendered/dev-timeline.png", width = 16 * 0.6, height = 9 * 0.6, bg = "white")
```

[High resolution plot](rendered/dev-timeline.png)

## Key events

Plot key events on a `timevis()` timeline. [Full page version](rendered/full-timeline.html).
...
key_events |>
  render_timevis(groups = dora_groups, file = "rendered/full-timeline.html", showZoom = TRUE)
```
