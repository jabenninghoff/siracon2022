...
- create_github_repo(): Create new GitHub repository following rdev conventions in the active user's
  account and create a basic package
...
monthly_releases |>
  ...
  geom_point() +
  geom_line() +
  geom_vline(xintercept = ymd_h("2022-01-18 0", tz = Sys.timezone()), linetype = "dotted") +
  theme_quo()
```

Also plot releases per commit, which will fall between 0 and 1.  The dotted line marks the
